...
const AdminProviderInner = ({ children }: { children: React.ReactNode }) => {
  ...
  const { fetchTableDataFromDb, user, isLoading } = railsContext;
  ...
  useEffect(() => {
    if (user && !isLoading && isAdmin) {
      ...
      // Create an array of tables to fetch
      ...
      const fetchAdminTables = async () => {
        try {
          ...
          const fetchPromises = tablesToFetch.map(async (tableName) => {
            ...
            return await fetchTableDataFromDb(tableName);
          });
  ...
  return (
    <AdminContext.Provider value={{ 
      ...
      listTables: [...ADMIN_TABLE_NAMES] // Override with admin-specific tables
    }}>
      {children}
